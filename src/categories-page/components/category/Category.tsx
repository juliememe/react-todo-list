...
export const Category = ({ setCategoryId, category }: CategoryProps) => {
  ...
  const showSubcategories = useMemo(() => {
    return subcategory ? (
      <Button
        handler={toggleSubcategory}
        btnClass={hideClassButton}
        btnType={"button"}
        disabled={false}
        ariaLabelText={"Show all subcategories button"}
        tooltipText={"Show all subcategories"}
      >
        <ArrowDownSquare />
      </Button>
    ) : null;
  ...
  return (
    <li
      className={cx({ category: true, openCategory: isActive })}
